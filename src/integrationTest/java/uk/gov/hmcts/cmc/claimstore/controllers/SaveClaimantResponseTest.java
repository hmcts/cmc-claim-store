...
public class SaveClaimantResponseTest extends BaseIntegrationTest {
    ...
    public void shouldSaveClaimantResponseRejectionWithNoFreeMediationAndNoPilotCourt() throws Exception {
        ...
        assertThat(claimantResponse.getDirectionsQuestionnaire()).isNotEmpty();
        verify(caseRepository, never()).saveCaseEvent(AUTHORISATION_TOKEN, claimWithClaimantResponse, REFERRED_TO_MEDIATION);
        verify(caseRepository, never()).saveCaseEvent(AUTHORISATION_TOKEN, claimWithClaimantResponse, ASSIGN_FOR_DIRECTIONS);
    }
